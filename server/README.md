...
**Сначала настройте вход по SSH без пароля** (один раз):

```bash
# Подставьте своего пользователя, если не ubuntu: root, ec2-user и т.д.
ssh-copy-id ubuntu@52.54.172.121
```

Потом из папки репозитория:

```bash
./server/deploy-to-server.sh
```

Если на сервере другой пользователь:

```bash
REMOTE=root@52.54.172.121 ./server/deploy-to-server.sh
```

Скрипт сам установит nginx (если нет), скопирует `dashboard.html` и конфиг, включит сайт и перезагрузит nginx.
